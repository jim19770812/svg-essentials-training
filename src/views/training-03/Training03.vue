...
<div>
    ...
    <h2>3.1 视口</h2>
    视口就是画布的区域，与html5中的viewport功能类似。
    <ul>
        <li>svg标签的width和height属性确定视口的大小</li>
        <li>视口单位的取值与css3规范一致（px,pt,em,rem,pc,cm,mm,in）</li>
    </ul>
    声明形式如下
    <pre>
        &lt;svg width="300" height="200"&gt;
        //使用像素未单位，不带单位就表示默认用px
        &lt;/svg&gt;
    ...
    <h2>3.3 为视口指定用户坐标</h2>
    <ul>
        <li>SVG通过viewBox属性为视口设置坐标系统，所谓坐标系统就是集合上的坐标系，用4个值来表示，中间用逗号或空格间隔，需要>=0才算有效</li>
        <li>0 0 100 200 就表示(0,0)(100,200)的一个矩形，坐标原点是(0,0)，其单位是相对于宽度和高度的一个近似值，比如viewBox的宽度是100，width=10cm,那么每个单位就是100/10=1cm</li>
        <li>设定了坐标系后不射定单位，实际单位会根据坐标系计算得来（之前默认是px，设定viewBox之后就是一个计算值(对宽度而言就是viewBox.width/width)）</li>
        <li>viewBox需要和width/height同时存在，坐标系需要实时计算</li>
        <li><strong>viewBox的加入，使得SVG可以很容易的等比放大了(调整宽度/高度就可以)</strong></li>
    </ul>
    <pre>
    ...
    <ul>
        <li>preserveAspectRatio指定对齐方式</li>
        <li>preserveAspectRatio="对齐方式 meet" 满足对齐，例如 preserveAspectRatio='yMinMin meet'</li>
        <li>preserveAspectRatio="对齐方式 slice" 切片对齐，例如 preserveAspectRatio='yMinMin slice'</li>
        <li>对齐方式还分水平方向和垂直方向两种
            ...
            <div>
                水平方向对齐
                <ul>
                    <li>xMin：vviewBox水平方向按照最小对齐（一般是左侧）</li>
                    <li>xMid：viewBox水平方向居中对齐</li>
                    <li>xMax：viewBox水平方向按照最大值对齐（一般是右侧）</li>
                </ul>
            </div>
        </li>
        <li>对齐方式要组合水平方向和垂直方向，所以一般是6位表示，<strong>需要注意的是6位中的垂直方向的Y要大写，水平方向的x要小写</strong>，需要写成xMinYMax, xMidYMin，写成xMinyMax， xMidyMid是不对的</li>
        <li>，需要</li>
    </ul>
    ...
    未指定preserveAspectRatio的样子（preserveAspectRatio默认是 <strong>"xMinYMin meet"</strong> ）
    <code>
    ...
    <h2>3.5 嵌套坐标系统</h2>
    <ul>
        <li>svg元素中还可以嵌入新的svg元素，新的svg使用自己的坐标系统</li>
        <li>应用嵌套坐标系统，3.4.3节的6张图合并成一张图
            <div>
                ...
                <ul>
                    <li>上例中大量使用了use标签，避免了重复画猫的代码，use xlink:href所指向的必须是一个g标签定义的部分，而g标签里包含了所有svg指令</li>
                    <li>每个嵌入的svg元素都可以设置偏移量x和y的值，不然所有图都会叠在一起</li>
                    <li>use xlink:href只能使用当前svg内部定义的图元，在本例中也是如此</li>
